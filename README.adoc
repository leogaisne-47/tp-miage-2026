...
#Léo Gaisne# (et #Etienne de Boysson#)
...
* Disposer d'un compte Github par personne (ou un pour deux personnes si vous êtes en binôme) et avoir https://github.com/settings/personal-access-tokens[créé un access token] de type `Fine-grained personal access tokens` avec le groupe de droits `Repo`.
...
Le code n'est pas structuré, l'architecture en couche demande qu'il soit divisé en trois couches: présentation, logique, et accès aux données. Le code actuelle ne l'est pas. Par example la classe "TodoItem" "Sert à la fois d'entité JPA et de DTO", dont données et transfer objet.

L'abscence de structure implique de la difficulté de maintenance, faible évolutivité et réutilisabilité. 
*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier se situe dans le TodoListController, avec la méthode finalContent().

*Cette règle est-elle facilement testable par un test unitaire ?*
La règle de gestion RG1 est une règle de gestion que l'infrastructure doit obligatoirement respecter. Il est donc très simple de le vérifier via un test unitaire. En revanche, dans ce cas précis, la visibilité de la méthode finalContent étant private, on est obligé de tester tout le contrôleur pour vérifier cette règle.
...
-it : Permet de voir les logs s'afficher en direct dans la console
- e : permet de définir une variable d'environnement (ici le mot de passe)
-p : Défini le port 5423:5432
postgres : le nom de l'image à pull

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
Car todoItemRepository n'est pas encore défini, en effet, il existe 2 constructeurs dans la classe, ce qui renvoi donc un objet vide ou null. La meilleure façon de l'injecter est donc de supprimer le constructeur vide. On doit aussi utiliser final pour s'assurer que la valeur soit affectée
...
Des tests de couche Web pour valider le mapping JSON et les codes de réponse HTTP et des tests d'intégration de données pour vérifier la validité des requêtes SQL et la communication avec la base de données.
